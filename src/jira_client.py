...
class JiraClient:
    ...
    def get_epics_and_stories(self):
        ...
        jql = f'({projects}) AND issuetype in (Epic, Story) AND due is not EMPTY ORDER BY due ASC'
        url = f"{self.base_url}/rest/api/3/search"
        params = {
            "jql": jql,
            "maxResults": 200,
            "fields": "summary,assignee,duedate,status,priority,description,comment,labels,issuetype,parent"
        }
        r = requests.get(url, auth=self.auth, headers=self.headers, params=params)
        r.raise_for_status()
        return r.json().get("issues", [])

    def get_last_comment_date(self, issue_key):
